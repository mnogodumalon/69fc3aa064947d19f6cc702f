...
export function ZimmerverwaltungDialog({ open, onClose, onSubmit, defaultValues, enablePhotoScan = true, enablePhotoLocation = true }: ZimmerverwaltungDialogProps) {
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      const schema = `{\n  "zimmer_name": string | null, // Zimmername / -nummer\n  "zimmer_typ": LookupValue | null, // Zimmertyp (select one key: "einzelzimmer" | "doppelzimmer" | "suite" | "freigehege") mapping: einzelzimmer=Einzelzimmer, doppelzimmer=Doppelzimmer, suite=Suite, freigehege=Freigehege\n  "kapazitaet": number | null, // Kapazität (Anzahl Katzen)\n  "tagespreis": number | null, // Tagespreis (€)\n  "beschreibung": string | null, // Beschreibung\n  "ausstattung": LookupValue[] | null, // Ausstattung (select one or more keys: "kletterbaum" | "fensterplatz" | "kuschelhoehle" | "spielzeug" | "heizung" | "kameraueberwachung" | "freigehege_zugang") mapping: kletterbaum=Kletterbaum, fensterplatz=Fensterplatz, kuschelhoehle=Kuschelhöhle, spielzeug=Spielzeug, heizung=Heizung, kameraueberwachung=Kameraüberwachung, freigehege_zugang=Freigehege-Zugang\n  "zimmer_status": LookupValue | null, // Status (select one key: "verfuegbar" | "belegt" | "in_reinigung" | "gesperrt") mapping: verfuegbar=Verfügbar, belegt=Belegt, in_reinigung=In Reinigung, gesperrt=Gesperrt\n}`;
